...
class CrimeAnalysisSystemTest {
    ...
    void setup() {
     ...
     //Ensures an incident is created successfully with the correct attributes.
    ...
    void testIncidentCreatedSuccessfully() throws SQLException {
        ...
        assertNotNull(retrievedIncident, "Incident should exist in the database");
        assertAll("Incident attributes should match",
                () -> assertEquals(101, retrievedIncident.getIncidentID(), "Incident ID should match"),
                () -> assertEquals("Burglary", retrievedIncident.getIncidentType(), "Incident type should match"),
                () -> assertEquals("Open", retrievedIncident.getStatus(), "Incident status should match")
        );
    }
    ...
    void testUpdateIncidentStatusSuccessfully() throws SQLException, IncidentNumberNotFoundException {
        db.createIncident(new Incident(102, "Robbery", new Date(), 15.50, 78.40,
                "Bank heist", "Open", 2, "SUS002", "OF456"));
        boolean updateResult = db.updateIncidentStatus(102, "Closed");
        assertTrue(updateResult, "Incident status update should return true");
        
        Incident updatedIncident = db.findIncidentById(102);
        assertNotNull(updatedIncident, "Incident should exist in the database");
        assertEquals("Closed", updatedIncident.getStatus(), "Incident status should be updated to Closed");
    }
} 
